...
describe('bookingPending', () => {
	test('attendee (request received) + organizer (review request), no ics', () => {
		...
		expect(attendee.content.heading).toBe('Your appointment request was received.');
		...
		expect(organizer.content.subject).toBe('Appointment request: 30 Minute Chat from Jane Doe');
		expect(organizer.content.heading).toBe('New appointment request');
