...
import { bookingRescheduledByAttendee } from './booking-rescheduled-by-attendee';
...
test('bookingRescheduledByAttendee: attendee envelope has links and a REQUEST ICS', () => {
	const [attendee] = bookingRescheduledByAttendee(input);

	expect(attendee.to).toBe('booker@example.com');
	expect(attendee.subject).toBe('Rescheduled: 30 Minute Chat with Jane Doe');
	expect(attendee.html).toContain('Your booking has been moved to a new time.');
	expect(attendee.html).toContain('token=tok&amp;cancel=1');
	expect(attendee.text).toContain(
		'Reschedule again: https://when.example.com/schedule/30-min?reschedule=appt-1&token=tok'
	);
...
test('bookingRescheduledByAttendee: organizer envelope notified of the reschedule, no ICS', () => {
	const [, admin] = bookingRescheduledByAttendee(input);

	expect(admin.to).toBe(validConfig.user.email);
	expect(admin.subject).toBe('Rescheduled: 30 Minute Chat with Booker');
	expect(admin.html).toContain('Booker &lt;booker@example.com> rescheduled 30 Minute Chat.');
	expect(admin.text).toContain('Booker <booker@example.com> rescheduled 30 Minute Chat.');
