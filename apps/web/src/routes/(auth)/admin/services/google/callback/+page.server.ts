...
export const load: PageServerLoad = async ({ url, cookies }) => {
	...
	if (denied) return { service: pending?.service ?? null, error: `Google returned "${denied}".` };
	...
	if (!pending || pending.state !== url.searchParams.get('state')) {
		return { service: null, error: 'This authorization did not match a pending connection.' };
	}

	const config = getConfig();
	const service = findGoogleService(config, pending.service);
	if (!service) {
		return { service: pending.service, error: `No google service named "${pending.service}".` };
	}
