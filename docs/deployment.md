...
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` / `GOOGLE_REFRESH_TOKEN` | a Google calendar | Generated by `pnpm setup-google`.                          |
...
- `pnpm setup-google` — opens your browser to authenticate with Google and prints the
  ...
  "Desktop app").
- `pnpm validate-config` — parses and validates `config.yaml` without starting the
  server; exits non-zero on errors.
