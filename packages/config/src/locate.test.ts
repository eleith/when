...
test('locates a nested scalar', () => {
	expect(locateInYaml(yaml, '/user/email')).toEqual({ line: 3, column: 10, missing: null });
});

test('locates an array element by index', () => {
	expect(locateInYaml(yaml, '/calendars/1/provider')).toEqual({
		line: 8,
		column: 15,
		missing: null
	});
...
test('locates the container itself', () => {
	expect(locateInYaml(yaml, '/calendars/1')).toEqual({ line: 7, column: 5, missing: null });
});
...
test('falls back to the nearest ancestor present in the file', () => {
	expect(locateInYaml(yaml, '/calendars/1/sync/refresh_every_minutes')).toEqual({
		line: 7,
		column: 5,
		missing: 'sync/refresh_every_minutes'
	});
});

test('an unresolvable pointer falls back to the document root', () => {
	expect(locateInYaml(yaml, '/meetings/0/slug')).toEqual({
		line: 1,
		column: 1,
		missing: 'meetings/0/slug'
	});
});

test('unescapes JSON Pointer tokens', () => {
	expect(locateInYaml(`a~b:\n  c: 1\n`, '/a~0b/c')).toEqual({ line: 2, column: 6, missing: null });
});
